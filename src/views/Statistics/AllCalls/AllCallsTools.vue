...
export default class AllCallsTools extends AppBase {
  ...
  set filterPeriod (val: string) {
    if (/\d{4}-\d{2}-\d{2}\|\d{4}-\d{2}-\d{2}/s.test(val)) {
      this.customPeriod = [];
      this.$store.commit('statistics/all_calls/filter/period', val);
    }
  }
  ...
  get refreshLoading () {
    return this.$store.getters['statistics/all_calls/pie_fetching'];
  }
  ...
  customPeriodWatchHandler (val: string[]) {
    ...
    if (val.length === 2) {
      ...
      if (d1.diff(d2, 'day') >= 0) {
        this.$store.commit('statistics/all_calls/filter/period', `${val[1]}|${val[0]}`);
      } else {
        this.$store.commit('statistics/all_calls/filter/period', `${val[0]}|${val[1]}`);
      }
    }
  }
