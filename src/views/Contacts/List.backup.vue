  ...
  <v-sheet>
    <v-row>
      ...
      >
        <app-tools>
          <template #left>
            <template v-if="contactsSelected.length === 0">
              <v-btn
                small
                tile
                text
            ...
            <!-- Импорт -->
            <template v-if="contactsSelected.length === 0 && $isGranted('contact.import_export')">
              <v-menu offset-y>
                  ...
                  <v-card-text class="px-0 py-0">
                    <v-list dense>
                      <template v-if="tagsSelectedForSet.length === 0">
                        ...
                        >
                          <v-list-item-title v-if="tagName && tagsAvailableForSet.length === 0">
                            "{{ tagName }}" {{ $tc('Create') }}
                          </v-list-item-title>
          ...
          <template #right>
            ...
            <app-pagination
              ...
              :disabled="contactsProcessLoading || dataTableContacts.selectedWhole"
            >
              <template #display>
                <v-menu offset-y>
                  <template #activator="{ on, attrs }">
...
export default Vue.extend<Data, Methods, Computed, Props>({
  ...
  computed: {
    ...
    paramFilters () {
      ...
      // Ответственный
      if (this.$data.filter.owner) {
        params.owner_id = this.$data.filter.owner.id
      }
    ...
    'dataTableContacts.selectedWhole': {
      handler (val: boolean) {
        ...
        if (val && JSON.stringify(this.paramFilters) !== '{}') {
          ...
          this.$data.contactsSelected = this.$data.dataTableContacts.items
        } else {
          this.unselectAll()
        }
      }
  ...
  methods: {
    ...
    initializeWatchForFilters () {
      ...
      this.$watch('filter.owner', (newVal: unknown & UserInterface) => {
        this.dataTableContacts.page = 1
        if (newVal) {
          this.$routerQuery.setQuery({
            owner_id: newVal.id
    ...
    onExportClick (format: 'excel' | 'csv') {
      ...
      const params: ContactExportParamsInterface = {
        ...
        format,
        target_contacts: this.dataTableContacts.selectedWhole ? [] : this.$data.contactsSelected.map((e: any) => e.id)
      }
    ...
    tagsSelectedApplyClick (items: ContactTagInterface[]) {
      ...
      if (items.length > 0) {
        ...
        const contactSelected = this.$data.contactsSelected.map((value: ContactInterface) => value.id)
        new Contacts()
          .setTags({
      ...
      const instance = await this.$dialog.show(SContactTransferDialog, {
        ...
        onTransfer: (scope: SContactTransferScopeInterface) => {
          const data: unknown & SContactTransferScopeInterface & { target_contacts: number[] } = {
            ...
            target_contacts: this.dataTableContacts.selectedWhole ? [] : this.contactsSelected.map((e: any) => e.id),
          ...
          new Contacts()
            ...
            .then((count: number) => {
              ...
              this.contactsSelected = [] // Отменить выделение всех контактов
        ...
        subtitle: this.$tc('No contacts selected | {n} contact selected | {n} contact selected | {n} contacts selected', this.contactsSelected.length),
        ...
        const instance = await this.$dialog.show(SContactTransferDialog, {
          ...
          onTransfer: (scope: SContactTransferScopeInterface) => {
            const params: unknown & SContactTransferScopeInterface & {
              ...
              filters: this.paramFilters,
              // Передайте пустой массив если хотите передать все контакты.
              target_contacts: this.dataTableContacts.selectedWhole ? [] : this.contactsSelected.map((e: ContactInterface) => e.id),
